...
public class BLServiceImplTest extends Assert {
  ...
  public void testCategoryUpdate() throws Exception {
    ...
    request.setData1("Cat1 - new");
    request.setData2("Category1 - new");
    request.setData3(user.getCategories().get(0).getId());

    blService.categoryUpdate(request);

    User userRead = userService.findByEMail("email");
    CategoryForTheUser category = userRead.getCategories().get(0);

    assertEquals(category.getName(), "Cat1 - new");
    assertEquals(category.getDescription(), "Category1 - new");
