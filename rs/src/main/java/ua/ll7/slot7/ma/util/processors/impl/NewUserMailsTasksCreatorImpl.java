package ua.ll7.slot7.ma.util.processors.impl;

import org.apache.log4j.Logger;
import org.springframework.stereotype.Component;
import ua.ll7.slot7.ma.model.User;
import ua.ll7.slot7.ma.util.processors.INewUserMailsTasksCreator;
import ua.ll7.slot7.ma.util.pubsub.events.UserSuccessfullyRegisterEvent;
import ua.ll7.slot7.ma.util.pubsub.listeners.IUserSuccessfullyRegisterEventListener;

/**
 * MA
 * Velichko A.
 * 31.01.15 16:27
 */
@Component
public class NewUserMailsTasksCreatorImpl implements INewUserMailsTasksCreator, IUserSuccessfullyRegisterEventListener {

  private static final Logger LOGGER = Logger.getLogger(NewUserMailsTasksCreatorImpl.class);

  @Override
  public void processMailsTasksForNewUser(User user) {
    LOGGER.debug("Create new email chain (email tasks) for new user : " + user.toString());
  }

  @Override
  public void onEvent(UserSuccessfullyRegisterEvent event) {
    processMailsTasksForNewUser(event.getUser());
  }
}
